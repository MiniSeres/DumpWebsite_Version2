const Dialog = {
    ...
    chooseFiles: (items, callback) => {
        ...
        Object.keys(extMap).forEach(ext => {
            ...
            total += info.count; totalSize += info.size;
            html += `
...
const UI = {
    msg: (t) => { const w = document.getElementById('msgArea'), el = document.createElement('div');
        el.className = 'msg';
        el.textContent = t;
        w.appendChild(el);
    ...
    addLog: (txt) => {
        ...
        if (logView) { logView.textContent = S.logs.join('\n'); logView.scrollTop = logView.scrollHeight; }
    },
    render: () => { const w = document.getElementById('queueList'), t = document.getElementById('totalCount'), d = document.getElementById('doneCount'), v = S.f === 'all' ? S.q : S.q.filter(x => x.s === S.f);
        t.textContent = S.q.length;
        const dc = S.q.filter(x => x.s === 'done').length;
        d.textContent = dc + ' xong';
        if (S.q.length) { const p = Math.round((dc / S.q.length) * 100); document.getElementById('progressBar').style.width = p + '%'; document.getElementById('progressText').textContent = p + '%'; }
        if (!v.length) { w.innerHTML = '<div class="empty-txt">Không có mục</div>'; return; }
        w.innerHTML = v.map((it, i) => {
            const idx = S.q.indexOf(it), cls = it.s === 'pending' ? 'pend' : it.s === 'run' ? 'run' : it.s === 'done' ? 'ok' : 'no', txt = it.s === 'pending' ? 'Chờ' : it.s === 'run' ? 'Đang xử lý' : it.s === 'done' ? 'Xong' : 'Lỗi', rt = it.retries > 0 ? `(${it.retries}/${C.RL})` : '';
            return `<div class="queue-item"><div class="item-top"><div style="display:flex;align-items:flex-start;gap:8px;flex:1;min-width:0;"><input type="checkbox" class="chk" data-i="${idx}"><span class="item-url" title="${it.url}">${it.url}</span></div><span class="tag tag-${cls}">${txt}${rt}</span></div><div class="item-foot"><span>${U.sz(it.size)}</span><span>${it.tm||'--'}</span><span>${it.checksum?it.checksum.substring(0,8)+'...':''}</span><span style="color:#64748b;">${it.fileType||'html'}</span><div style="display:flex;gap:4px;"><button class="s-btn" onclick="window._v(${idx})">Xem</button><button class="s-btn" onclick="window._d(${idx})">Tải</button><button class="s-btn" onclick="window._r(${idx})">Thử lại</button><button class="s-btn danger" onclick="window._del(${idx})">Xóa</button></div></div></div>`;
        }).join('');
        document.querySelectorAll('.chk').forEach(cb => cb.addEventListener('change', UI.updSel));
    },
    updSel: () => { const c = document.querySelectorAll('.chk:checked'); document.getElementById('selectedCount').textContent = c.length; document.getElementById('batchActions').style.display = c.length ? 'flex' : 'none'; },
    stats: () => {
        const t = S.q.length, d = S.q.filter(x => x.s === 'done').length, f = S.q.filter(x => x.s === 'fail').length, p = S.q.filter(x => x.s === 'pending').length, r = S.q.filter(x => x.s === 'run').length;
        ...
        if (d > 0) { document.getElementById('avgTime').textContent = U.fmt(Math.round(S.tt / d)); }
        if (S.start > 0 && S.run) {
            ...
            if (rem > 0 && d > 0) { const rate = d / (e / 1000); document.getElementById('etaTime').textContent = U.fmt((rem / rate) * 1000); }
        }
        if (S.start > 0 && S.run) {
            const elapsed = (Date.now() - S.start) / 1000;
            if (elapsed > 0) { const speed = S.crawled / elapsed; document.getElementById('speedDisplay').textContent = speed.toFixed(1); }
        }
    },
    show: t => { document.getElementById('codeView').textContent = t || '--'; },
    clear: () => { document.getElementById('codeView').textContent = ''; },
    toggleLog: (show) => { document.getElementById('logContainer').style.display = show ? 'block' : 'none'; }
};
...
const W = {
    process: async () => {
        ...
        if (!crawlOn) { S.run = false; S.start = 0; UI.msg('⏸ Crawl đang tắt'); UI.addLog('⏸ Crawl đang tắt'); return; }
        const items = S.q.map((it, i) => ({ it, i })).filter(({ it }) => it.s === 'pending' || (it.s === 'fail' && it.retries < C.RL));
        if (!items.length) { S.run = false; S.start = 0; UI.msg('✅ Hoàn thành'); UI.addLog('✅ Hoàn thành tất cả'); UI.stats(); return; }
        S.run = true;
        ...
        UI.stats(); UI.render();
        if (!S.pause && S.q.some(x => x.s === 'pending' || (x.s === 'fail' && x.retries < C.RL))) { await U.sleep(50); W.process(); } else { S.run = false; if (!S.pause) { UI.msg('✅ Xong queue'); UI.addLog('✅ Xong queue'); } }
    },
    processItem: async (idx) => {
        ...
        UI.addLog('⏳ Đang xử lý: ' + it.url + ' (' + (it.fileType || 'html') + ')');
        try {
            ...
            if (cached) {
                ...
                S.dc++; S.crawled++;
                UI.msg('⚡ Cache: ' + it.url);
                UI.addLog('⚡ Cache: ' + it.url + ' (' + U.sz(it.size) + ')');
                S.proc.delete(idx); UI.render(); UI.stats(); return;
            }
            ...
            it.tm = used + 'ms';
            S.tt += used; S.dc++; S.crawled++;
            UI.msg('✅ Xong: ' + it.url);
            UI.addLog('✅ Xong: ' + it.url + ' (' + U.sz(it.size) + ') - ' + used + 'ms');
        } catch (e) {
            it.s = 'fail';
            it.err = e.message;
            it.retries = (it.retries || 0) + 1;
            UI.addLog('❌ Lỗi: ' + it.url + ' - ' + e.message + ' (lần ' + it.retries + ')');
            if (it.retries < C.RL) {
                UI.msg('🔄 Thử lại ' + it.retries + '/' + C.RL + ': ' + it.url);
                setTimeout(() => { if (!S.pause && it.s === 'fail') { it.s = 'pending'; UI.render(); W.process(); } }, 1000 * it.retries);
            } else { UI.msg('❌ Lỗi: ' + it.url); }
        } finally { S.proc.delete(idx); UI.render(); UI.stats(); }
    },
    crawlAll: async () => {
        if (S.run) { Dialog.alert('⚠️ Đang crawl, dừng trước khi crawl mới!'); return; }
        const val = document.getElementById('urlInput').value.trim();
        const urls = Net.extract(val);
        if (!urls.length) { Dialog.alert('⚠️ Không có URL hợp lệ!'); return; }
        Dialog.alert('🔍 Đang crawl tất cả file từ ' + urls.length + ' trang...');
        UI.addLog('🔍 Bắt đầu crawl ALL từ ' + urls.length + ' trang');
        ...
        for (const u of urls) {
            try {
                ...
                total++;
                UI.addLog('✅ Đã crawl xong: ' + u);
                UI.render(); UI.stats();
            } catch (e) { UI.addLog('❌ Lỗi crawl ' + u + ': ' + e.message); }
        }
        S.duplicates += dupCount;
        UI.msg('🔍 Tìm thấy ' + total + ' file (bỏ qua ' + dupCount + ' trùng)');
        UI.addLog('🔍 Tổng: ' + total + ' file mới, ' + dupCount + ' trùng');
        UI.render(); UI.stats();
        if (total > 0) { S.pause = false; await W.process(); }
    },
    start: async () => {
        if (S.run) { Dialog.alert('⚠️ Đang crawl, vui lòng đợi!'); return; }
        const val = document.getElementById('urlInput').value.trim();
        const urls = Net.extract(val);
        if (!urls.length) { Dialog.alert('⚠️ Không có URL hợp lệ!'); return; }
        ...
        if (!added) { Dialog.alert('⚠️ Không có URL mới để crawl!'); return; }
    ...
    retry: (idx) => {
        ...
        it.s = 'pending'; it.retries = 0; it.err = null;
    ...
    retryAll: () => {
        let c = 0;
        S.q.forEach(it => { if (it.s === 'fail') { it.s = 'pending'; it.retries = 0; it.err = null; c++; } });
    ...
    clearAll: () => {
        Dialog.confirm('Xóa tất cả dữ liệu?', () => {
            ...
            UI.render(); UI.stats(); UI.clear();
        });
    },
    view: (idx) => { const it = S.q[idx]; UI.show(it?.txt || '--'); },
    download: (idx) => {
        const it = S.q[idx];
        if (!it?.txt) { Dialog.alert('Không có nội dung để tải!'); return; }
        const dm = U.dm(it.url), ext = it.fileType || 'txt';
    ...
    downloadSelected: () => {
        ...
        if (!checked.length) { Dialog.alert('Chọn ít nhất 1 file để tải!'); return; }
        const items = [];
        checked.forEach(cb => {
        ...
        if (!items.length) { Dialog.alert('Không có file nào đã crawl xong!'); return; }
        Dialog.chooseFiles(items, (selected) => {
            if (selected.length === 1) { const idx = S.q.indexOf(selected[0]); W.download(idx); return; }
            const zip = new JSZip();
            ...
            }).catch(e => { Dialog.alert('❌ Lỗi tạo ZIP: ' + e.message); });
        });
    },
    downloadAll: async () => {
        const items = S.q.filter(x => x.s === 'done' && x.txt);
        if (!items.length) { Dialog.alert('Không có dữ liệu để tải!'); return; }
        Dialog.chooseFiles(items, (selected) => {
            if (selected.length === 1) { const idx = S.q.indexOf(selected[0]); W.download(idx); return; }
            const zip = new JSZip();
            for (const it of selected) {
    ...
    copy: () => {
        ...
        if (!t.trim()) { Dialog.alert('Không có nội dung để sao chép!'); return; }
        navigator.clipboard.writeText(t).then(() => UI.msg('📋 Đã sao chép')).catch(() => Dialog.alert('❌ Lỗi sao chép!'));
    },
    selAll: () => { document.querySelectorAll('.chk').forEach(c => c.checked = true); UI.updSel(); },
    selNone: () => { document.querySelectorAll('.chk').forEach(c => c.checked = false); UI.updSel(); },
    selInv: () => { document.querySelectorAll('.chk').forEach(c => c.checked = !c.checked); UI.updSel(); },
    batchDel: () => {
        const c = document.querySelectorAll('.chk:checked');
        if (!c.length) { Dialog.alert('Chọn ít nhất 1 mục!'); return; }
    ...
    batchRetry: () => {
        ...
        [...c].forEach(cb => {
            const idx = parseInt(cb.dataset.i);
            const it = S.q[idx];
            if (it && (it.s === 'fail' || it.s === 'done')) { it.s = 'pending'; it.retries = 0; it.err = null; count++; }
        });
    ...
    import: (file) => {
        ...
        r.onload = e => { document.getElementById('urlInput').value = e.target.result; UI.msg('📂 Import: ' + file.name); UI.addLog('📂 Import file: ' + file.name); };
        r.onerror = () => Dialog.alert('❌ Lỗi đọc file!');
        r.readAsText(file);
    },
    export: () => {
        const txt = S.q.map(x => x.url).join('\n');
        if (!txt) { Dialog.alert('Danh sách trống!'); return; }
    ...
    exportJson: () => {
        const items = S.q.filter(x => x.s === 'done' && x.txt);
        if (!items.length) { Dialog.alert('Không có kết quả!'); return; }
        const data = items.map(x => ({ url: x.url, size: x.size, checksum: x.checksum, time: x.tm, content: x.txt, type: x.fileType }));
    ...
    clearLog: () => { S.logs = []; document.getElementById('logView').textContent = ''; UI.msg('🗑 Xóa log'); }
};
...
const init = () => {
    ...
    document.getElementById('filePick').addEventListener('change', e => { W.import(e.target.files[0]); e.target.value = ''; });
    document.getElementById('clearCache').addEventListener('click', () => { Cache.clear(); UI.msg('🗑 Xóa cache'); UI.addLog('🗑 Xóa cache'); });
    document.getElementById('crawlAllBtn').addEventListener('click', W.crawlAll);
    document.getElementById('showLogBtn').addEventListener('click', () => { UI.toggleLog(true); UI.addLog('📋 Mở log'); });
    document.getElementById('hideLogBtn').addEventListener('click', () => { UI.toggleLog(false); UI.addLog('📋 Đóng log'); });
    ...
    if (saved) {
        try {
            ...
            if (Array.isArray(p) && p.length) { S.q = p; UI.render(); UI.stats(); UI.msg('📂 Khôi phục ' + S.q.length + ' mục'); UI.addLog('📂 Khôi phục ' + S.q.length + ' mục từ cache'); }
        } catch {}
    }
    setInterval(() => {
        try {
            ...
            localStorage.setItem('crawlState', JSON.stringify({ run: S.run, pause: S.pause, crawled: S.crawled, total: S.q.length }));
        } catch {}
        UI.stats();
    }, 5000);
